...
(() => {
  ...
  const ICONES = {
    ...
    mais: '<svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true"><path d="M12 5v14M5 12h14" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/></svg>'
  };
  function icone(nome) {
  ...
  const btnAdicionar = $('#abrir-compositor');
  const btnCancelar = $('#cancelar-compositor');
  const btnOk = $('#confirmar-produto');
  const tituloCompositor = $('#titulo-compositor');
  const etapaFormulacao = $('#etapa-formulacao');
  const etapaDose = $('#etapa-dose');
  const dica = $('#dica');
  ...
  let destaqueId = null;
  let editandoItemId = null;   // item da mistura em edição (null = novo produto)
  let nomeConfirmado = false;  // libera a etapa "formulação"
  ...
  function linhaItem(item, numero) {
    ...
    const editar = () => abrirCompositor(item);
    return el('li', { class: 'passo' + (item.id === destaqueId ? ' passo--novo' : ''), 'data-id': item.id },
      el('span', { class: 'passo__num', texto: String(numero) }),
      el('div', {
        class: 'passo__corpo', role: 'button', tabindex: '0',
        'aria-label': `Editar ${item.nome}, ${formatarDose.format(item.dose)} ${item.unidade}`,
        onclick: editar,
        onkeydown: (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); editar(); } }
      },
  ...
  function renderizarMistura() {
    ...
    dica.hidden = mistura.length === 0;
    destaqueId = null;
  }

  function removerItem(id) {
    if (editandoItemId === id) fecharCompositor(false);
  ...
  tornarConfirmavel(btnLimpar, {
    ...
    aoConfirmar: () => { fecharCompositor(false); mistura = []; salvarMistura(); renderizarMistura(); }
  });

  /* ---------- Adicionar / editar (uma etapa por vez) ---------- */
  function lerDose() {
  ...
  // Nome → libera Formulação → escolher formulação libera Dose + OK → OK só ativa com tudo preenchido
  function atualizarEtapas() {
    etapaFormulacao.hidden = !nomeConfirmado;
    etapaDose.hidden = !(nomeConfirmado && selForm.value);
    btnOk.disabled = !(campoNome.value.trim() && selForm.value && lerDose() > 0);
  }
  ...
  function abrirCompositor(item = null) {
    limparCampos();
    editandoItemId = item ? item.id : null;
    tituloCompositor.textContent = item ? 'Editar produto' : 'Novo produto';
    if (item) {
      campoNome.value = item.nome;
      selForm.value = String(item.pos);
      campoDose.value = String(item.dose).replace('.', ',');
      selUnidade.value = item.unidade;
      unidadeManual = true;
      nomeConfirmado = true;
    }
    form.hidden = false;
    ...
    atualizarEtapas();
    if (item) tituloCompositor.focus({ preventScroll: true });
    else campoNome.focus();
    form.scrollIntoView({ block: 'nearest', behavior: reduzMovimento() ? 'auto' : 'smooth' });
  }

  function fecharCompositor(devolverFoco = true) {
    limparCampos();
    editandoItemId = null;
    form.hidden = true;
    btnAdicionar.hidden = false;
    if (devolverFoco) btnAdicionar.focus({ preventScroll: true });
  }
  ...
  function confirmar() {
    ...
    const dados = {
      ...
      pos: Number(selForm.value),
      dose: lerDose(),
      unidade: selUnidade.value
    };
    let item = editandoItemId ? mistura.find((i) => i.id === editandoItemId) : null;
    if (item) Object.assign(item, dados);
    else { item = { id: novoId(), ...dados }; mistura.push(item); }
  ...
  btnAdicionar.append(icone('mais'), el('span', { texto: 'Adicionar produto' }));
  btnAdicionar.addEventListener('click', () => abrirCompositor());
  btnCancelar.addEventListener('click', () => fecharCompositor());
  ...
  selForm.addEventListener('change', () => {
    ...
    atualizarEtapas();
    if (selForm.value && !campoDose.value) campoDose.focus();
  });
  ...
  campoNome.addEventListener('input', () => { atualizarSugestoes(); atualizarEtapas(); });
  campoNome.addEventListener('keydown', (e) => {
    const aberta = !sugestoesEl.hidden;
    const n = sugestoes.length;
    if (aberta && e.key === 'ArrowDown') { e.preventDefault(); marcarSugestao((sugestaoAtiva + 1) % n); }
    else if (aberta && e.key === 'ArrowUp') { e.preventDefault(); marcarSugestao((sugestaoAtiva - 1 + n) % n); }
    else if (aberta && e.key === 'Escape') { e.stopPropagation(); fecharSugestoes(); }
    else if (e.key === 'Enter') {
      e.preventDefault();
      if (aberta && sugestaoAtiva >= 0) return aplicarProduto(sugestoes[sugestaoAtiva]);
      if (!campoNome.value.trim()) return;
      // "Próximo": libera a formulação e leva o foco até ela (ou até a dose, se o produto já era conhecido)
      nomeConfirmado = true;
      atualizarEtapas();
      selForm.focus();
      if (selForm.value) campoDose.focus();
    }
  });
  // Ao sair do campo com um nome preenchido, a etapa seguinte aparece logo abaixo
